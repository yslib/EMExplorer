...
MRCDataModel::MRCDataModel(const QSharedPointer<MRC> &data):
	AbstractSliceDataModel(data->getSliceCount(),data->getWidth(),data->getHeight()),
	m_d(data)
{
...
{
	int width = m_d->getWidth();
	int height = m_d->getHeight();
