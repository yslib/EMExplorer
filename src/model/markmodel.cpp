	...
	m_identity(dataModel)
{
	...
	m_rootItem->setModelIndex(createIndex(0, 0, m_rootItem));
 ...
 */
QList<TreeItem*> MarkModel::treeItems(const QModelIndex & parent, TreeItemType type)
{
...
void MarkModel::removeSelectedItems()
{
	while (!selectionModel()->selection().indexes().isEmpty())
	{
        ...
        TreeItem* parent = treeItem(index.parent());
        removeRow(index.row(), index.parent());
        if(parent->type() == TreeItemType::Instance && parent->childCount() == 0)
            //delete instance too
        {
...
/**
 * \brief Reimplemented from QAbstractItemModel::insertRows(int row, int count, const QModelIndex & parent)
 */
...
/**
 * \brief Reimplemented from QAbstractItemModel::removeRows(int row, int count, const QModelIndex & parent)
 */
