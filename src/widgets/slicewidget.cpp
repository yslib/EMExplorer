	...
	for (const auto & item : items) {
		...
		if (itm == m_slice) {	 // Operations must perform on slice item.
			...
			if (m_state == Operation::Paint) {
                ...
                addingMark = new AddMarkCommand(m_currentPaintingSlice);
                addingMark->setMark(m_pen);
                addingMark->addPoint(itemPoint);
				event->accept();
				return;
			}
...
void SliceWidget::mouseReleaseEvent(QMouseEvent *event)
{
	...
	if (m_state == Operation::Paint)			//create a mark
	{
        ...
        addingMark->addPoint(m_currentPaintingSlice->mapFromScene(mapToScene(event->pos())));
		if (m_currentPaintingSlice == m_slice) {
            if (addingMark->size() > 3)
                emit markAdded(addingMark);
			else
            {
                delete addingMark;
                addingMark = nullptr;
