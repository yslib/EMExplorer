...
public:
	...
	template<typename T>
	static Float dotProduct(const Vector3<T> & v1, const Vector3<T> & v2)
	{
		return Dot(v1, v2);
	}
...
template<typename T, typename U> inline Vector3<T> operator*(const U & s, const Vector3<T> & v)
{
	return v * s;
}
...
public:
	...
	{
		assert(!isNaN(s));
		return Point3<T>::dotProduct(*this, p);
	}
...
template<typename T, typename  U> inline Point3<T> operator*(const U & s, const Point3<T> & p)
{
	return p * s;
}
